...
public class JJB{
  ...
  public static void main( String[] args ){
    ...
    // Connect to Database
    dbase = new DataBase( botAdmin, new File( botDB ) );
    dbase.connect();

    // Create and Start Queu
    queue = new SQLiteQueue( new File( botDB ) );
    queue.start();

    // Start Input Processor
    ip = new InputProcessor( queue );

    // connect the Bot
    Bot bot = new Bot( botJid, botPass, botResource, botAuth, botAdmin, botPort, ip );
    bot.login();
